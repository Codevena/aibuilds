...
- [x] Create an exact temporary tooling directory with `AIBUILDS_COPY_TOOLS_DIR=$(mktemp -d)`, install `typescript@5.9.3` and `parse5@7.3.0` there, and run `AIBUILDS_COPY_TOOLS_ROOT="$AIBUILDS_COPY_TOOLS_DIR" node scripts/generate-copy-manifest.cjs docs/superpowers/evidence/2026-08-10-copy-manifest.before.txt`.
- [x] Verify the unchanged five UI sources have no diff against `b9859b1`, the artifact has 937 lines, and SHA-256 is `61e5316b4db604efaf936344158fcfe0fefde5acd5962c72e833a9b95e9bec9a`.
- [x] Remove only the Plan Gate’s `.review/` workspace, then commit the reviewed design delta, plan, generator and before artifact:
...
- [x] Record both printed identities in the SDD progress ledger immediately and in this plan’s Task-8 Findings Mapping before final commit. No Task-4 copy edit may begin unless this provenance commit exists.
...
- [x] **Step 1: Add the failing path-policy tests**
...
- [x] **Step 4: Replace every ad-hoc World read/list/static decision**
...
- [x] **Step 5: Harden dependencies and remove `uuid`**
...
- [x] **Step 6: Verify GREEN and mutation coverage**
...
- [x] **Step 1: Write failing classification tests with literal fixtures**
...
- [x] **Step 3: Implement parse5 traversal and deterministic classification**
...
- [x] **Step 4: Implement structural external-link hardening**
...
- [x] **Step 5: Verify GREEN and mutations**
...
- [x] **Step 1: Extend moderation tests before state changes**
...
- [x] **Step 3: Implement persisted moderation state**
...
- [x] **Step 4: Add failing publication-flow tests**
...
- [x] **Step 5: Integrate contribution and startup flows**
...
- [x] **Step 6: Add the risky-to-safe route regression**
...
- [x] **Step 7: Add authenticated admin decisions and HTTP tests**
...
- [x] **Step 8: Verify GREEN and mutations**
...
- [x] **Step 2: Verify identity RED, implement, and verify GREEN**
...
- [x] **Step 4: Verify metrics RED, implement, and integrate**
...
- [x] **Step 5: Add a behavioral public-contract test**
...
- [x] **Step 6: Align MCP and all agent-facing docs**
...
- [x] **Step 7: Add the rendered public-copy guard**
...
- [x] **Step 8: Verify contracts and mutations**
...
- [x] **Step 1: Write failing pure season tests**
...
- [x] **Step 3: Implement deterministic derivations**
...
- [x] **Step 4: Persist curation events and wire mutations**
...
- [x] **Step 5: Add APIs and MCP context**
...
- [x] **Step 6: Verify GREEN and mutations**
...
- [x] **Step 1: Write failing replay-controller tests**
...
- [x] **Step 3: Implement minimal UMD replay state machine**
...
- [x] **Step 4: Add dashboard markup and data states**
...
- [x] **Step 6: Update landing narrative**
...
- [x] **Step 7: Verify GREEN and mutations**
...
- [x] **Step 1: Verify the immutable pre-edit copy evidence**
...
- [x] **Step 2: Write failing UI and SEO contract tests**
...
- [x] **Step 4: Implement accessibility fixes**
...
- [x] **Step 5: Implement two-agent SEO promotion**
...
- [x] **Step 6: Generate and classify the post-edit copy manifest**
...
- [x] **Step 8: Visual browser verification**
...
- Add: `.superpowers/sdd/2026-08-10-hardening-revival/final-smoke.cjs`
- Add: `.superpowers/sdd/2026-08-10-hardening-revival/final-mutations.cjs`
- Add: `.superpowers/sdd/2026-08-10-hardening-revival/task-8-report.md`
- Modify outside repository: `/Users/markus/Documents/Brain/02 Projekte/Experimente/AiBuilds.md`
- Modify outside repository: `/Users/markus/Documents/Brain/05 Daily Notes/2026-08-11.md`
...
- [x] **Step 1: Run focused and full automated verification**
  ...
  Final evidence on `2a841fd` plus the documentation handoff delta: `npm test` passed 184/184
  with 0 fail/skip; all 14 listed syntax checks exited 0; both production audits reported
  `found 0 vulnerabilities`; `git diff --check` was empty.

- [x] **Step 2: Run security and live smoke checks**
  ...
  Final isolated smoke: 9/9 private/traversal/dot/hidden/quarantined paths returned 404;
  4/4 safe assets returned 200; 11/11 CSP directives were present and `allow-same-origin`
  absent; missing challenge returned 403; risky medical content quarantined with zero public
  contribution/achievement frames; safe content published and broadcast. Every Stats field
  matched an independent `/api/history` + `/api/files` count (quarantine count from the private
  temporary moderation state), and repository World/data digests were identical before/after.
  Command: `node .superpowers/sdd/2026-08-10-hardening-revival/final-smoke.cjs`.

- [x] **Step 3: Run final mutation report**

  Consolidate every task’s individually executed named temporary mutations from its task report,
  recording the task-wide focused command and WITH/WITHOUT result matrix in this plan. Then repeat
  at least one load-bearing named mutation per task on a disposable copy of the final tree. Required
  final record: every report-matrix guard has WITH ≥1 and WITHOUT =0, and every final-tree
  representative has WITH ≥1 and WITHOUT =0. Restore or discard each temporary mutation before the
  next and confirm `git diff --check`.

  The per-task reports retain the mutation definitions, focused runner methodology and observed
  quantities for all 102 closeout mutants (Task totals 11/12/8/5/25/22/19); each was executed
  individually with `WITHOUT=0` and `WITH>=1`. Their durable focused commands are:

  | Task | Focused command used for the task’s named mutations |
  | --- | --- |
  | 1 | `node --test test/world-files.test.js` |
  | 2 | `node --test test/content-governance.test.js` |
  | 3 | `node --test test/moderation.test.js test/publication-flow.test.js test/admin-quarantine.test.js` |
  | 4 | `node --test test/mcp-identity.test.js test/platform-metrics.test.js test/public-contract.test.js test/public-copy.test.js` |
  | 5 | `node --test test/seasons.test.js test/moderation.test.js` |
  | 6 | `node --test test/replay.test.js test/platform-metrics.test.js` |
  | 7 | `node --test test/seo-publication.test.js test/web-ui-contract.test.js` |

  On a disposable copy of the current final tree, Task 8 additionally repeated one load-bearing
  named mutation per task with the command form
  `node --test --test-name-pattern='<pattern>' <focused-test>`.

  | Task | Current-tree mutation / focused test pattern | WITHOUT | WITH |
  | --- | --- | ---: | ---: |
  | 1 | dot privacy removed / `private and traversal paths never resolve` | 0 | 1 |
  | 2 | medical branch disabled / `medical dosing instructions are quarantined` | 0 | 1 |
  | 3 | pre-read byte cap removed / `oversized moderation state is rejected before its bytes are read` | 0 | 1 |
  | 4 | live boundary made exclusive / `liveness includes exactly the 15-minute boundary` | 0 | 1 |
  | 5 | votes read barrier removed / `curation mutations hide provisional reads` | 0 | 1 |
  | 6 | Replay cap raised to 51 / `full replay payload is rejected beyond 50 events` | 0 | 1 |
  | 7 | SEO threshold lowered to one / `page publication requires two visible agents` | 0 | 1 |

  Exact runner: `node .superpowers/sdd/2026-08-10-hardening-revival/final-mutations.cjs`;
  result `7/7`, every mutant exactly one failure. The disposable copy was removed, production
  files were never mutated, and the post-run diff check stayed clean.

- [x] **Step 4: Request two independent reviews**
...
- [x] **Step 7: Final commit and push**
  ...
  git add -f \
    .superpowers/sdd/2026-08-10-hardening-revival/final-smoke.cjs \
    .superpowers/sdd/2026-08-10-hardening-revival/final-mutations.cjs \
    .superpowers/sdd/2026-08-10-hardening-revival/task-8-report.md
...
- Task 1: `9bc2f21`, `232b2ad`, `c7b85c6`; private-path/symlink boundary and dependency
  hardening; final focused 6/6, full 22/22, audit 0, review clean.
- Task 2: `079f782`, `8dab9aa`; fail-closed high-stakes/promotion Governance and external-link
  hardening; focused 19/19, full 41/41, 12 mutations killed, review clean.
- Task 3: `a328db6` through `70fb74f`; durable quarantine/approval, public-record boundary,
  atomic publication and fail-closed Git-repair WAL; focused 96/96, full 121/121, final eight
  WAL mutations killed, audit 0, review clean.
- Task 4: `2959f0c`, `8e2e602`; stable private MCP identity, positive write policy, truthful
  operator copy and visible-only metrics; focused 14/14, full 135/135, audits 0, review clean.
- Task 5: `4a00793`, `ec36109`; UTC Daily Seasons, roles, curation, Replay/Hall contracts and
  atomic read/write state boundary; focused 64/64, full 144/144, 11 fix mutations killed,
  audits 0, two reviews PASS.
- Task 6: `f9406bd`, `9c478c0`; truthful live/offline/replay UI, authoritative payload
  sanitation, race-safe refreshes and timed/BFCache freshness; focused 33/33, full 173/173,
  eight fix mutations killed, audits 0, two reviews PASS.
- Task 7: `2a841fd`; complete accessibility/mobile contracts and staged two-agent SEO;
  focused 11/11, full 184/184, 19 named mutations killed, audits 0, 15/15 browser matrix with
  zero final console errors, both final reviews PASS.
- Task 8: full-system gate complete on `2a841fd` plus the handoff delta: full 184/184,
  14 syntax checks, both production audits 0, isolated security/live smoke PASS, 102 reported
  closeout mutants plus 7/7 final-tree representatives killed, and both final reviews PASS.
  Brain handoff is recorded in `[[AiBuilds]]` and `[[2026-08-11]]`. The final handoff commit is
  `docs: finalize hardening revival handoff`; `main` was pushed to `origin/main` and equality was
...
- After: 1,169 AST entries; SHA-256
  `a23506ae43c642584b8d4a5bb5e5a0e92c08a1b8416d355cf1b5e9fa6ec2f8a1`.
  Task-7 normalized delta: 17 removals / 41 additions; every visible Task-7 addition is
  category 7 accessibility copy, with non-rendered analytics/template tokens documented in
  `docs/superpowers/evidence/2026-08-10-copy-manifest-diff.md`.

### Final Reviews

- Reviewer A: final full-system initial FAIL (2 README CRITICAL, 2 README WARN); first delta
  FAIL (1 evidence-contract CRITICAL); final delta PASS with 0 CRITICAL/WARN.
- Reviewer B: final full-system initial FAIL (tracked evidence/overclaim CRITICAL plus README
  contract deltas); final delta PASS with 0 CRITICAL/WARN. Fresh independent evidence:
  184/184, 14 syntax checks, both audits 0, copy manifest exact and final mutations 7/7.
